...
import SubscribedCard from "../cards/subscribedtweets";
...
function Home() {
  ...
  const [allsubscribedtweets, setAllSubscribedTweets] = useState([]);
  const GetAllMySubscribedTweets = async () => {
    const response = await axios
      .get(`${API_BASE_URL}/allsubscribedtweets`, CONFIG_OBJ)
      //debugger;
      .then((res) => setAllSubscribedTweets(res.data))
      .catch((err) => console.error(err));
  };
  useEffect(() => {
    GetAllMySubscribedTweets();
  }, []);
              ...
              .map((tweet) => {
                return (
                  ...
                  >
                    ...
                    <SubscribedCard
                      tweetData={tweet}
                      GetAllMySubscribedTweets={GetAllMySubscribedTweets}
                    />{" "}
                  </div>
                );
              })
